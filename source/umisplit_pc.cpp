...
{
    public:
        splitFiles(bool _compressed,bool _filter,string outputDir, vector<string> barcodeWells, string R1sten){
            isCompressed=_compressed
            keepNonMatches=_filter
            for(int j=0;j<NWELLS;j++){
                ...
                if(!keepNonMatches && !j){
                    ...
                    file =outputDir+"/"+barcodeWells[j-1]+"/"+R1stem+"R2_"+barcodeWells[j-1]+"_"+std::to_string(numberofFiles[j])+".fq";
                    if (compressFlag){
                        file=file+".gz";
        ...
        void write(char *lineR1, char *linesR2, int wellIndex){
            char buffer[1024];
            ...
            *bufPtr++=':';
            memcpy(bufPtr,linesR2,UMILength);
            bufPtr+=UMILength;
            *bufPtr++='\n';
            //copy the rest of the lines to buffer
            dest=*linesR2;
            while(*dest){
                *bufPtr++=*dest++;	
            }
            if isCompressed {
                #pragma omp critical
                gzwrite(ofpgzs[wellIndex+1],buffer,bufPtr-buffer);
            }
            else{
                #pragma omp critical
                fwrite(ofps[wellIndex+1],buffer,bufPtr-buffer);                
            }    
        }
...
int main(int argc, char *argv[]){
    ...
    #pragma omp parallel{
        ...
        for (int i=0;i<inputFiles.size()/2;i++){
            ...
            if ( thread  == producerThread%nThreads,maxThread) parseFile(thread,inputFiles[2*i],inputFiles[2*i+1]);
            producerThread++
        }
    }
    return 0;  
}
bool parseFile(int producerThread, string fileR1, string fileR2, int maxSizeKB, string barcodeFilename, int mismatchTol, int NTol ){
    #pragma omp single{
        //producer reads files
        //make a barcodePanel for each thread
        #if NWELLS > 256
        ...
        kseq_t *seq1,*seq2;
        int l1,l2;
        fp1 = gzopen(fileR1.c_str(), "r");
        if(fp1 == Z_NULL){
            fprintf(stderr,"unable to open %s\n",fileR1.c_str());
            exit(EXIT_FAILURE);
        }
        fp2 = gzopen(fileR2.c_str(), "r");	
        if(fp2 == Z_NULL){
            fprintf(stderr,"unable to open %s\n",fileR2c_str());
            exit(EXIT_FAILURE);
        }
        ...
        //variables for different wells files
...
gzFile getCompressedFilePointer(string file){
    return 
}
